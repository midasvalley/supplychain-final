...
def generate_summary_report(nutrition_df, inventory_data, consumption_data, wastage_data, order_data):
    """Generate a summary report with key metrics"""
    script_dir = Path(__file__).parent
    
    # Convert Total Cost to numeric, properly handling the dollar sign and commas
    # First create a copy to avoid modifying the original
    cost_df = nutrition_df.copy()
    
    # Convert Total Cost column to string first to ensure consistent handling
    cost_df['Total Cost'] = cost_df['Total Cost'].astype(str)
    
    # Remove dollar signs and commas
    cost_df['Total Cost'] = cost_df['Total Cost'].str.replace('$', '', regex=False).str.replace(',', '', regex=False)
    
    # Convert to float, ignoring errors
    cost_df['Total Cost'] = pd.to_numeric(cost_df['Total Cost'], errors='coerce')
    
    # Calculate total cost
    total_cost = cost_df[cost_df['Week'] != 'TOTAL COST']['Total Cost'].sum()
    
    # Count weeks
    weeks = [week for week in nutrition_df['Week'] if isinstance(week, (datetime, pd.Timestamp)) or (isinstance(week, str) and 'TOTAL' not in week)]
    total_weeks = len(weeks)
    
    # Count weeks with orders
    weeks_with_orders = sum(1 for week in weeks if week in order_data and order_data[week])
    order_frequency = (weeks_with_orders / total_weeks) * 100 if total_weeks > 0 else 0
    
    # Calculate average weekly cost
    avg_weekly_cost = total_cost / total_weeks if total_weeks > 0 else 0
    
    # Calculate consumption and wastage
    total_consumed = 0
    for week_data in consumption_data.values():
        total_consumed += sum(week_data.values())
    
    total_wasted = 0
    for week_data in wastage_data.values():
        total_wasted += sum(week_data.values())
    
    wastage_percentage = (total_wasted / (total_consumed + total_wasted)) * 100 if (total_consumed + total_wasted) > 0 else 0
    
    # Generate report
    report = f"""Supply Chain Analysis Summary
===========================

Total Cost: ${total_cost:.2f}
Average Weekly Cost: ${avg_weekly_cost:.2f}
Total Weeks Analyzed: {total_weeks}
Weeks with Orders: {weeks_with_orders}
Order Frequency: {order_frequency:.1f}%

Food Consumption and Waste Metrics
===============================
Total Servings Consumed: {total_consumed}
Total Servings Wasted: {total_wasted}
Wastage Percentage: {wastage_percentage:.2f}%
"""
    
    # Write report to file
    with open(script_dir / 'summary_report.txt', 'w') as f:
        f.write(report)
    
    print(f"Summary report generated: {script_dir / 'summary_report.txt'}")
...
def main():
    ...
    generate_summary_report(nutrition_df, inventory_data, consumption_data, wastage_data, order_data)
    ...
    print("5. summary_report.txt - Text summary of key supply chain metrics")
